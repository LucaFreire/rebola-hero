...
# Música
#mixer.music.load(path.join("Game Assets", "background_music.mp3"))  # Add música de fundoa
...
def Pressione_Start():
    Enter_Start = Press_Start_Fonte.render("Pressione ENTER P/ Iniciar")  # Mensagem
    Tela.blit(Enter_Start, (130, 280))  # Localização da Mensagem


# Pontuação
Pontos = 0
Fonte = pygame.font.Font(path.join("Game Assets", "slkscr.ttf"), 36) # Estilo de Fonte
TextoX = 10 # Tamanho ou Localização da Fonte
TextoY = 10 # Tamanho ou Localização da Fonte

def Mostrar_Pontos(x, y):
    Ponto = Fonte.render("Pontuação: " + str(Pontos), True, (0, 255, 0))
    Tela.blit(Ponto, (x, y))


# Recorde
Recorde = 0 #shelve.open('game_data')
AltoX = 10
AltoY = 100
...
class Controle_Jogo:
    ...
    def Update(self, dt):
        if self.state.quit:
            ...
            self.Troca()
        self.state.update(self.Tela, dt)
